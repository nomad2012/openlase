...
void DoCubes(float limit)
{
	...
	{
		if (ctime*AB < 15)
			xpos = -2.0 +2*sinf(ctime*AB * M_PI * 0.5 / 15.0);
		...
		olDrawString(font, -0.6*(ctime-1.0), 0.35, 0.3, C_WHITE, "Software: OpenLase by marcan");
		olDrawString(font, -0.5*(ctime-2.0), -0.60, 0.3, C_WHITE, "Galvos courtesy of TomG");
		olDrawString(font, -0.9*(ctime-3.0), 0.70, 0.3, C_WHITE, "Music: 'abandon' by Stamen");
                /*
...
int scale_offset_ild_points(IldaFile *ild, float scale_factor, float x_offset, float y_offset)
{
	IldaPoint *p = ild->points;
	int i;
	for (i = 0; i < ild->count; i++) {
          ild->points[i].x = ild->points[i].x * scale_factor + x_offset;
          ild->points[i].y = ild->points[i].y * scale_factor + y_offset;
	}
}
...
void DoLogo(char *filename)
{
	IldaFile *ild;
	ild = olLoadIlda(filename);
...
void DoFadeText(char *s, float s_h, float limit, float fade_time)
{
	...
	float s_x = -olGetStringWidth(font, s_h, s) / 2.0;
	float s_y = -1.0;

	for(s_y = -1.0; s_y <= (s_h/2.0); s_y += 0.1) {
	...
	while(audiotime < limit) {
		...
		render();
                ...
                if (audiotime > (limit - fade_time)) {
                  printf("audiotime = %f limit = %f fade_time = %f bright = %f ftime = %f\n", audiotime, limit, fade_time, bright, ftime);
                  bright = (limit - audiotime) / fade_time * 255.0f;
                  if (bright < 0.0f) {
                    bright = 0.0f;
                  }
                }
...
void DoScrollText(char *s, float s_h, float limit, float scroll_time)
{
	float cur_draw = 0;
	float ctime = 0;
	...
	float s_y = -1.0;
        ...
        float start_time = audiotime;

        while (audiotime < (start_time + scroll_time)) {
          s_y = (s_h / 2.0) - (start_time + scroll_time - audiotime) / scroll_time;
          olDrawString(font, s_x, s_y, s_h, C_WHITE, s);
          ctime += render();
	}

        while (audiotime < (limit - scroll_time)) {
          olDrawString(font, s_x, s_y, s_h, C_WHITE, s);
          ctime += render();
        }

	while (audiotime < limit) {
          s_y = 1.0 + s_h - (limit - audiotime) / scroll_time;
          s_y = (s_y <= (s_h / 2.0)) ? (s_h / 2.0) : s_y;
          olDrawString(font, s_x, s_y, s_h, C_WHITE, s);
          ctime += render();
	}
}

void DoScrollText2(char *s, char *s2, float s_h, float limit, float scroll_time)
{
	float cur_draw = 0;
	float ctime = 0;
	...
	float s_x = -olGetStringWidth(font, s_h, s) / 2.0;
	float s_y = -1.0;
	float s2_x = -olGetStringWidth(font, s_h, s2) / 2.0;

        float start_time = audiotime;

        while (audiotime < (start_time + scroll_time)) {
          s_y = s_h - (start_time + scroll_time - audiotime) / scroll_time;
          olDrawString(font, s_x, s_y, s_h, C_WHITE, s);
          olDrawString(font, s2_x, s_y - s_h, s_h, C_WHITE, s2);
          ctime += render();
	}
...
void DoTitle(float limit)
{
	IldaFile *ild;
        float scale = 0.05;
	ild = olLoadIlda("mmslogo.ild");
        scale_offset_ild_points(ild, 1.0, 0.0, 0.2);
        scale_offset_ild_points(ild, scale, 0.0, 0.0);
	...
	while(1) {
		...
		cur_draw += ftime * count / 3.0;
                if (scale < 0.8) {
                  scale *= 1.05;
                  scale_offset_ild_points(ild, 1.05, 0.0, 0.0);
                }
	...
	const char *s="Milwaukee Makerspace";

	float s_x = -olGetStringWidth(font, 0.22, s) / 2.0;
	float s_y = -0.6;
	float s_h = 0.22;
...
int main (int argc, char *argv[])
{
	...
	//DoStars(96);
	DoTitle(40.0);
        DoScrollText2("Milwaukee", "Makerspace", 0.4, 52.0, 2.0);
        DoScrollText("presents", 0.4, 64.0, 2.0);
        DoScrollText("LAYZOR!!!", 0.5, 74.0, 0.5);
        DoScrollText2("a project by", "Dennis Draheim", 0.3, 84.0, 2.0);
